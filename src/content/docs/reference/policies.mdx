...
description: Comprehensive reference for Firefox enterprise policies used to configure and manage enterprise deployments.
sidebar:
...
<Icon name="warning" color="red" class="icon-inline" /> `Default`: determines whether
cookies are accepted at all. (_Deprecated_. Use `Behavior` instead)
...
`ExpireAtSessionEnd` determines when cookies expire. (_Deprecated_. Use [`SanitizeOnShutdown`](#sanitizeonshutdown-selective) instead)
