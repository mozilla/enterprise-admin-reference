...
description: "Configure built-in Data Loss Prevention rules that warn on or block data actions per domain."
...
- `Rules`: The list of DLP rules. Each rule takes:
  ...
  - `Domains`: (**required**) The domains the rule applies to, at least one.
  ...
  - `Message`: The text shown to the user when the rule triggers.
