...
<PolicyExample policy="ContentAnalysis" />

## Values

- `AgentName`: the name of the DLP agent. This is used in dialogs and notifications about DLP operations. The default is `A DLP Agent`.
- `AgentTimeout`: the timeout in number of seconds after a DLP request is sent to the agent. After this timeout, the request will be denied unless `TimeoutResult` is set to 1 or 2. The default is 300.
- `AllowUrlRegexList`: a space-separated list of regular expressions that indicates URLs for which DLP operations will always be allowed without consulting the agent.
...
- `ClientSignature`: the required signature of the DLP agent connected to the pipe. If this is a non-empty string and the DLP agent does not have a signature with a Subject Name that exactly matches this value, Firefox will not connect to the pipe. The default is the empty string.
- `DefaultResult`: the desired behavior for DLP requests if there is a problem connecting to the DLP agent. The default is 0.
  Possible values are:
  - `0`: Deny the request (default)
  - `1`: Warn the user and allow them to choose whether to allow or deny
  - `2`: Allow the request
- `DenyUrlRegexList`: a space-separated list of regular expressions that indicates URLs for which DLP operations will always be denied without consulting the agent. The default is the empty string.
...
- `InterceptionPoints` controls settings for specific interception points.
  - `Clipboard`: controls clipboard paste operations for files and text (for copy operations, see `ClipboardCopy`).
    - `Enabled`: indicates whether clipboard paste operations should use DLP. The default is `true`.
    - `PlainTextOnly`: indicates whether to only analyze the text/plain format on the clipboard.
  ...
  - `ClipboardCopy`: controls web content copying data to the clipboard (for paste operations, see `Clipboard`).
    - `Enabled`: indicates whether clipboard copy operations should use DLP.
      The default is false, so that existing DLP deployments are unaffected.
    - `PlainTextOnly`: indicates whether to only analyze the text/plain format being copied.
      If this value is false, all formats being copied will be analyzed, which some DLP agents may not expect.
      Regardless of this value, files being copied will be analyzed as usual. The default is `true`.
  - `Download`: controls download operations. (Added in Firefox 142, Firefox ESR 140.2)
    - `Enabled`: indicates whether download operations should use DLP. The default is false.
  - `DragAndDrop`: controls drag and drop operations for files and text.
    ...
    - `PlainTextOnly`: indicates whether to only analyze the text/plain format in what is being dropped.
      If this value is false, all formats will be analyzed, which some DLP agents may not expect.
      Regardless of this value, files will be analyzed as usual. The default is `true`.
  - `FileUpload`: controls file upload operations for files chosen from the file picker.
    - `Enabled`: indicates whether file upload operations should use DLP. The default is `true`.
  - `Print`: controls print operation.
...
- `PipePathName`: the name of the pipe the DLP agent has created and Firefox will connect to. The default is `path_user`.
- `ShowBlockedResult`: indicates whether Firefox should show a notification when a DLP request is denied. The default is `true`.
- `TimeoutResult`: the desired behavior for DLP requests if the DLP agent does not respond to a request in less than `AgentTimeout` seconds. The default is `0`.
  Possible values are:
  - `0`: Deny the request (default)
  - `1`: Warn the user and allow them to choose whether to allow or deny
  - `2`: Allow the request
...
Software\Policies\Mozilla\Firefox\ContentAnalysis\InterceptionPoints\ClipboardCopy\Enabled = 0x0 | 0x1
Software\Policies\Mozilla\Firefox\ContentAnalysis\InterceptionPoints\ClipboardCopy\PlainTextOnly = 0x1 | 0x0
