...
**Compatibility:** Firefox 139, Firefox ESR 60 (POST support in Firefox ESR 68, Encoding support in Firefox 91, `Remove` in Firefox ESR 60.2)\
...
Software\Policies\Mozilla\Firefox\SearchEngines\DefaultPrivate = NAME_OF_SEARCH_ENGINE
...
`DefaultPrivate`: OMA-URI:

```url
./Device/Vendor/MSFT/Policy/Config/Firefox~Policy~firefox~Search/SearchEngines_DefaultPrivate
```
...
```xml
<enabled/>
<data id="SearchEngines_DefaultPrivate" value="NAME_OF_SEARCH_ENGINE"/>
```
