...
- `Enabled`: A Boolean. If `true`, the event is recorded. The default is `false`.
...
As of version 157, `SecurityLogging` events defaults to `false`, meaning `Enabled` must be explicitly set to `true` for the relevant telemetry to be sent.
